...
public class TheMazeII {
   ...
   public void dfs(int [][]maze,int []cur,int[] destination,int [][]dists){
        int i=cur[0],j=cur[1];
        ...
        for(int []dir:dirs){
            int x=i,y=j;
            int dist=dists[x][y];
            while(x>=0&&y>=0&&y<n&&x<m&&maze[x][y]!=1){
                x+=dir[0];
                y+=dir[1];
                dist++;
            }
            ...
            dist--;
                    if(dists[x][y]>dist) {
                        dists[x][y]=dist;
                        dfs(maze, new int[]{x, y}, destination, dists);
                    }
        }
   }
    ...
    public int shortestDistanceBFS(int[][] maze, int[] start, int[] destination) {
        ...
        while(!q.isEmpty()){
            ...
            for(int []dir:dirs){
                ...
                if (dists[x][y]>dist){
                    dists[x][y]=dist;
                    if (x != destination[0] || y != destination[1]) q.add(new int[]{x, y});
            }
            }
        }
        int res= dists[destination[0]][destination[1]];
